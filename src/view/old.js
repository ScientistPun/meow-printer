...
createApp({
  setup() {
    // ==================== 文件相关状态 ====================
    const files = ref([]);           // 当前选择的文件列表 [{path, name}]
    const fileName = ref('');         // 显示的文件名（多个文件时显示数量）
    ...
    const selectMode = ref(false);     // 是否处于多选模式
    const selectedHistoryFiles = ref([]); // 已选中的历史文件
    ...
    const autoSelectMedia = async (fileData) => {
      try {
        ...
        if (res.ok) {
          ...
          console.debug('Detected dimensions:', dims);
          const matched = matchFileToMedia(dims.width, dims.height);
          console.debug('Matched media:', matched);
    ...
    const previewPrint = async () => {
      ...
      try {
        ...
        // 微信浏览器使用新窗口打开，其他浏览器使用 iframe 预览
        if (isWechatBrowser()) {
          // 微信浏览器：创建下载链接让用户在新窗口查看
          const blobUrl = URL.createObjectURL(blob);
          const fileName = encodeURIComponent(files.value[0]?.name || 'preview.pdf');
          window.open(blobUrl, '_blank');
          previewLoading.value = false;
        } else {
          // 其他浏览器：使用 iframe 全屏预览
          const blobUrl = URL.createObjectURL(blob);
