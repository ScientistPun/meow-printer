...
import { getPrinters, printFile, getJobs, cancelJob, getPrinterCapabilities, createTextPdf, getMediaSizeMM, getAvailableFonts } from './cups.js';
...
app.post('/api/print', upload.single('file'), async (req, res) => {
  try {
    ...
    if (!options.media && (!options.mediaWidth || !options.mediaHeight)) {
      const { getFileDimensions, matchToMediaSize, getMediaSizeMM } = await import('./cups.js');
...
app.post('/api/preview', upload.single('file'), async (req, res) => {
  try {
    ...
    if (['.jpg', '.jpeg', '.png', '.gif', '.webp', '.tiff', '.bmp'].includes(ext)) {
      ...
      const { scalePdf } = await import('./cups.js');
...
app.get('/api/file/dimensions', async (req, res) => {
  try {
    ...
    const { getFileDimensions } = await import('./cups.js');
